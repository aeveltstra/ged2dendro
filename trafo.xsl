...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
  ...
  <xsl:key name="children" match="family" use="child"/>
  ...
  <xsl:template match="header">
    <xsl:element name="diagram">
      ...
      <xsl:element name="mxGraphModel">
        ...
        <xsl:attribute name="arrows">0</xsl:attribute>
  ...
  <xsl:template match="spouse" mode="name">
    <xsl:copy-of select="key('people-by-id', current()/text())[1]/name[1]/text()"/>
    <xsl:choose>
  ...
  <xsl:template match="family">
    ...
    <xsl:element name="mxCell">
      ...
      <xsl:attribute name="value"><xsl:apply-templates select="spouse" mode="name"/></xsl:attribute>
      <xsl:choose>
        <xsl:when test="endNode">
          <xsl:attribute name="style">strokeWidth=2;html=1;shape=mxgraph.flowchart.terminator;whiteSpace=wrap;fillColor=#B5739D;strokeColor=#381410;fontColor=#ffffff;fontStyle=3;</xsl:attribute>
        </xsl:when>
  ...
  <xsl:template match="person">
    <xsl:element name="mxCell">
      <xsl:attribute name="id"><xsl:value-of select="id/text()"/></xsl:attribute>
      <xsl:attribute name="value"><xsl:value-of select="name[1]/text()"/></xsl:attribute>
      <xsl:choose>
        <xsl:when test="endNode">
          <xsl:attribute name="style">strokeWidth=2;html=1;shape=mxgraph.flowchart.terminator;whiteSpace=wrap;fillColor=#FFFF99;strokeColor=#023406;fontColor=#000000;fontStyle=3;</xsl:attribute>
        </xsl:when>
        <xsl:when test="andThisNode">
          <xsl:attribute name="style">ellipse;whiteSpace=wrap;html=1;fillColor=#99FFCC;strokeColor=#427326;fontColor=#000000;fontStyle=1;</xsl:attribute>
        </xsl:when>
        <xsl:when test="own-family and not(own-family/text() = '') and (key('families-by-id', current()/own-family/text())[*][child])">
          <xsl:attribute name="style">rounded=1;whiteSpace=wrap;html=1;fillColor=#fff2cc;strokeColor=#d6b656;fontColor=#000000;</xsl:attribute>
        </xsl:when>
	<xsl:otherwise>
          <xsl:attribute name="style">ellipse;whiteSpace=wrap;html=1;fillColor=#99CCFF;strokeColor=#82b366;fontColor=#000000;</xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:attribute name="parent">1</xsl:attribute>
      <xsl:attribute name="vertex">1</xsl:attribute>
